...
export function flat<T extends { children?: T[] }>(arr: T[], depth = MAX_DEPTH): T[] {
  ...
  arr.forEach(({ children, ...item }) => {
    ...
    if (depth > 0 && children?.length) {
      result.push(...flat(children, depth - 1));
    }
  });
  return result;
}
 ...
 */
export function toMap<T>(arr: T[], key: keyof T | ((item: T) => string)): Map<string, T> {
  return new Map<string, T>(arr.map((item) => [typeof key === 'function' ? key(item) : String(item[key]), item]));
}
...
export function toTree<T extends { children?: T[] }>(
  ...
  // 将数组转换为map，以便快速查找和操作
  const map = toMap(clonedArr, key);
  // 遍历每个元素，根据parentKey将其添加到正确的父节点下，或作为根节点
  clonedArr.forEach((item) => {
    // 根据parentKey获取当前元素的父节点标识
    const key = typeof parentKey === 'function' ? parentKey(item) : String(item[parentKey] ?? 'root');
